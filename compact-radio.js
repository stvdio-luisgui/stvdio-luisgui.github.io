(() => {
  ...
  function load(index, autoplay = true) {
    ...
    frame.src = `https://www.youtube.com/embed/${id}?rel=0&playsinline=1${autoplay ? "&autoplay=1" : ""}${origin}`;
  }

  select.addEventListener("change", () => load(select.selectedIndex, true));
  previous.addEventListener("click", () => load(select.selectedIndex - 1, true));
  next.addEventListener("click", () => load(select.selectedIndex + 1, true));
})();